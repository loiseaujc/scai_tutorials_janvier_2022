...
\section{A (very) brief overview of SVD}
\begin{frame}
  \sectionpage
\end{frame}
...
\begin{frame}
  ...
  \movie[autostart, loop, width=\textwidth, poster]{
    \includegraphics[width=\textwidth]{cylinder_flow_dns}
    }{imgs/cylinder_flow_dns.mp4}
...
%   \vfill
%   \centering
%   \includegraphics[width=.9\textwidth]{cats_dogs}
%
%   \textcolor{black}{\textbf{Is it a cat or a dog ?}}
%   \vfill
% \end{frame}
%
% }
...
%   \vfill
%   \centering
%   \includegraphics[height=.9\textheight]{eigenpets}
%   \vfill
% \end{frame}
%
% }
...
%   \vfill
%   \centering
%   \includegraphics[width=\textwidth]{class_separation}
%   \vfill
% \end{frame}
%
% }
...
%   \begin{minipage}{.68\textwidth}
%     \begin{itemize}
%       \item Increases the interpretability of the classifier by highlighting the most important relevent part of the images.
%
%       \bigskip
%
%       \item Compress down the information to the most important pixels, enabling the deployment onto low-memory devices for IoT.
%
%       \bigskip
%
%       \item Applicable to any linear classifier.
%     \end{itemize}
%   \end{minipage}%
%   \hfill
%   \begin{minipage}{.28\textwidth}
...
%   \begin{minipage}{.68\textwidth}
%     \begin{itemize}
%       \item Applicable only to linear classifier.
%
%       \bigskip
%
%       \item Performances degrade more rapidly than that of the full-order model when noise is present.
%     \end{itemize}
%   \end{minipage}%
%   \hfill
%   \begin{minipage}{.28\textwidth}
...
\begin{frame}
  ...
  \begin{tikzpicture}[overlay, remember picture, >=stealth, nodes={align=left, inner ysep=1pt}, <-]
    ...
    \path (a.south) ++ (0, -2em) node[anchor=north east, color=red!67] (U){Output};
    \draw [color=red!87] (a.south) |- ([xshift=-0.3ex, color=red] U.south west);
    
    \path (b.south) ++ (0, -2em) node[anchor=north west, color=blue!67] (Sigma){Input};
    \draw [color=blue!87] (b.south) |- ([xshift=-0.3ex, color=blue] Sigma.south east);
    
    \path (c.south) ++ (0, -4em) node[anchor=north west, color=OliveGreen!67] (V){Unknown conv. kernel};
    \draw [color=OliveGreen!87] (c.south) |- ([xshift=-0.3ex, color=OliveGreen] V.south east);
  \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{tabular}{c|r|r}
    ...
    $\vdots$ & $\vdots$ & $\vdots$ \\
    $\bm{u}_k$ & $\displaystyle \sum_{i=1}^k \bm{A}^{i-1} \bm{Bu}_{k-i}$ & $\displaystyle \sum_{i=1}^k \bm{CA}^{i-1} \bm{Bu}_{k-i} + \bm{Du}_k$
  \end{tabular}
...
\begin{frame}
  ...
  {\Large
    ...
    \[
      \bm{y}_k = \bm{Du}_k + \sum_{i=1}^k \bm{CA}^{i-1} \bm{Bu}_{k-i}
    \]
...
\begin{frame}
  ...
  \includegraphics[width=\textwidth]{conv_model_data}

  \textbf{Input/Output data for system identification}
...
\begin{frame}
  ...
  \(
    \displaystyle \minimize_{\bm{h}} \quad \| \bm{y} - \bm{h} * \bm{u} \|_2^2
  \)
...
%
% \section{A (very) brief introduction to optimal control and reinforcement learning}
% \begin{frame}
%   \sectionpage
% \end{frame}
%
...
%         \[
%         \begin{aligned}
%           \bm{x}_{i+1} & = \bm{f}(\bm{x}_i, \bm{u}_i) \\
%           \bm{y}_i & = \bm{g}(\bm{x}_i, \bm{u}_i)
%         \end{aligned}
%         \]
%             };
%
...
%   \centering
%
%   \underline{\textbf{Infinite-horizon LQR problem}}
%
%   \bigskip
%
%   {
...
%     \Large
%     \[
%       V(\bm{x}_i) = \dfrac{1}{2} \sum_{k=i}^{\infty} \bm{x}_k^T \bm{Qx}_k + \bm{u}_k^T \bm{Ru}_k
%     \]
%
%     \onslide<2>
%     \Large
%     \[
%     V(\bm{x}_i) = \dfrac{1}{2} \bm{x}_i^T \bm{P} \bm{x}_i
%     \]
%
%     \onslide<3>
%     \Large
...
%     \Large
%     \[
%       V(\bm{x}_i) = \dfrac{1}{2} \left( \bm{x}_i^T \bm{Qx}_i + \bm{u}_i^T \bm{Ru}_i \right) + \dfrac{1}{2} \sum_{k=i+1}^{\infty} \bm{x}_k^T \bm{Qx}_k + \bm{u}_k^T \bm{Ru}_k
%     \]
%
%     \onslide<5>
%     \Large
%     \[
%       V(\bm{x}_i) = \dfrac{1}{2} \left( \bm{x}_i^T \bm{Qx}_i + \bm{u}_i^T \bm{Ru}_i \right) + V(\bm{x}_{i+1})
%     \]
%
%     \onslide<6>
%     \Large
%     \[
%     \dfrac{1}{2} \bm{x}_i^T \bm{Px}_i = \dfrac{1}{2} \left( \bm{x}_i^T \bm{Qx}_i + \bm{u}_i^T \bm{Ru}_i \right) + \dfrac{1}{2} \bm{x}_{i+1}^T \bm{Px}_{i+1}
%     \]
%
%
%     \onslide<7>
%     \Large
%     \[
%     \dfrac{1}{2} \bm{x}^T \bm{Px} = \dfrac{1}{2} \left( \bm{x}^T \bm{Qx} + \bm{u}^T \bm{Ru} \right) + \dfrac{1}{2} \left( \bm{Ax} + \bm{Bu} \right)^T \bm{P} \left( \bm{Ax} + \bm{Bu} \right)
%     \]
%
%   \end{overprint}
%   }
...
%   \vfill
%   \centering
%   \underline{\textbf{Optimality condition}}
%
%   {\Large
%   \[
%   \bm{u} = - \left( \bm{R} + \bm{B}^T \bm{PB} \right)^{-1} \bm{B}^T \bm{PAx}
%   \]
%   }
%
%   \vfill
...
%     coltext=white,
%     colback=black,
%     title=\textbf{Discrete-time Algebraic Ricatti Equation},
%     frame style tile={width=\paperwidth}{background.jpg}
%     ]
%
%   \bigskip
...
%   {\large
%   \[
%   \bm{P} = \bm{Q} + \bm{A}^T \bm{PA} - \bm{A}^T \bm{PB} \left( \bm{R} + \bm{B}^T \bm{PB} \right)^{-1} \bm{B}^T \bm{PA}
%   \]
%   }
%
%   \bigskip
%
%   DARE is a nonlinear matrix equation.
%   Solving direct it requires $\mathcal{O}(n^3)$ operations, making it intractable, even for medium-sized systems.
%
%   \end{tcolorbox}
%
%   \vfill
    ...
    \begin{frame}
      ...
      \begin{tikzpicture}[
        ...
        fill=black,
          ...
          \textcolor{white}{\textbf{Black Box}}
        };
        ...
        \node[input, name=tmp] at (-3, -2) {};
        
        \draw[->, thick] (observer) -- (controller) node [midway, above] {$\hat{\bm{x}}$};
        \draw[->, thick] (controller.north) |- (plant.west) node [midway, above right] {$\bm{u}$};
        \draw[->, thick] (plant.east) -| (observer.60) node [midway, above left] {$\bm{y}$};
        \draw[->, thick] (tmp) -| (observer.120);
        
        \draw[dashed, ultra thick] (-5, -1.5) -- (5, -1.5) -- (5, -4.5) -- (-5, -4.5) -- cycle;
        \node[] at (-4.35, -4.25) {\textbf{LQG}};
